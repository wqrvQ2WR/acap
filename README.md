...
| `acap music` | Add background music to a video (mix under original audio or replace it) |
...
| `acap music` | 영상에 배경음악 추가 (원본 소리 위에 믹스 또는 교체) |
...
### music — Background Music / 배경음악

Add background music to a video. By default the music is mixed under the
original audio and looped to fill the whole video.

영상에 배경음악을 넣습니다. 기본적으로 원본 소리 위에 음악을 깔고, 영상 길이에 맞춰
음악을 자동 반복합니다.

```bash
acap music video.mp4 bgm.mp3                  # mix under original audio / 원본 위에 믹스
acap music video.mp4 bgm.mp3 --volume 0.15    # quieter music / 음악 더 작게
acap music video.mp4 bgm.mp3 --replace        # replace original audio / 원본 소리 교체
acap music video.mp4 bgm.mp3 --no-loop        # play music once, then silence / 반복 안 함
acap music video.mp4 bgm.mp3 -o out.mp4
```

| Flag | Default | Description |
|------|---------|-------------|
| `-v, --volume` | `0.3` | Music volume multiplier (0.0 ~ 1.0+) |
| `--replace` | off | Replace the original audio with the music instead of mixing |
| `--no-loop` | off | Don't loop the music; it plays once and the rest is silent |

> The video stream is copied without re-encoding — only audio is processed, so it's fast.
> 비디오는 재인코딩 없이 복사되고 오디오만 처리하므로 빠릅니다.
...
│   ├── music.go       # music command (background music)
...
└── internal/
    ...
    ├── ai.go          # edit analysis
    ├── music.go       # background music mixing
